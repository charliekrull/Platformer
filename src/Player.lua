...
function Player:checkLeftCollisions(dt)
    local tileTopLeft = self.map:pointToTile(self.x + 1, self.y + 1) --1 pixel on each side for leniency in fitting through gaps
    local tileBottomLeft = self.map:pointToTile(self.x + 1, self.y + self.height - 1)


    --place player outside the x bounds of the tile to reset any overlap
    if (tileTopLeft and tileBottomLeft) and (tileTopLeft:collidable() or tileBottomLeft:collidable()) then
        self.x = (tileTopLeft.x - 1) * TILE_SIZE + tileTopLeft.width - 1 --convert to world coordinates, then set self.x

    else
        --allow us to walk on top of solid objects
        self.y = self.y - 1
        local collidedObjects = self:checkObjectCollisions()
        self.y = self.y + 1

        --reset X if new collided objects
...
function Player:checkRightCollisions(dt)
    ...
    if (tileTopRight and tileBottomRight) and (tileTopRight:collidable() or tileBottomRight:collidable()) then
        ...
        --allow us to walk on solid objects
        self.y = self.y - 1
        local collidedObjects = self:checkObjectCollisions()
        self.y = self.y + 1
